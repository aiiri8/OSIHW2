...
void sigfunc(int sig) {
    ...
    munmap(ptr, sizeof(Book) + 2 * sizeof(sem_t));
    shm_unlink(SHM_NAME);
    exit(0);
}
...
int main(int argc, char** argv) {
    ...
    ftruncate(fd, sizeof(Book) + 2 * sizeof(sem_t));
    ...
    mutex_read = mmap(NULL, sizeof(sem_t), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    sem_init(mutex_read, 1, 0);
    mutex_write = mmap(NULL, sizeof(sem_t), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    sem_init(mutex_write, 1, 1);
